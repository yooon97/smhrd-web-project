...
List<Post> folpostList = (List)session.getAttribute("folpostList");
System.out.print("결과 둥둥 : "+folpostList.get(0).getPost());
%>
...
var mapContainer = document.getElementById('map'), // 지도를 표시할 div 
    mapOption = { 
    center: new kakao.maps.LatLng(<%=folMarkerList.get(0).getLATITUDE()%>, <%=folMarkerList.get(0).getLONGITUDE()%>), // 지도의 중심좌표
    level: 3 // 지도의 확대 레벨
    };  
...
linePath.push(new kakao.maps.LatLng(<%=folMarkerList.get(i).getLATITUDE()%>, <%=folMarkerList.get(i).getLONGITUDE()%>))
<%};%>

<% for(int i =0; i<folMarkerList.size()-1; i++) {%>
positions.push({title: '<div>제목 : <%=folpostList.get(i).getTitle()%></div><div>작성일자 : <%=folpostList.get(i).getPost_dt()%></div>', 
latlng: new kakao.maps.LatLng(<%=folMarkerList.get(i).getLATITUDE()%>, <%=folMarkerList.get(i).getLONGITUDE()%>)})
<%};%>

<% for(int i =0; i<folMarkerList.size()-1; i ++) {%>
shapes.push(<%=folMarkerList.get(i).getMarker_shape()%>)
<%}%>
