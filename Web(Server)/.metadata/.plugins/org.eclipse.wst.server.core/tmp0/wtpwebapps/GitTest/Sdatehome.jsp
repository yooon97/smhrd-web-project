...
<script>
	...
	<% List<Marker> markerList = (List)session.getAttribute("markerList");
		//System.out.print("뽑혔나요? " + marList.get(0).getLATITUDE());
		System.out.print("사이즈 : " + markerList.size());
		
		List<Post> postList = (List)session.getAttribute("postList");
		System.out.print("날짜별 : "+postList.get(0).getPost());%>

	

var mapContainer = document.getElementById('map'), // 지도를 표시할 div 
    mapOption = { 
    center: new kakao.maps.LatLng(<%=markerList.get(0).getLATITUDE()%>, <%=markerList.get(0).getLONGITUDE()%>), // 지도의 중심좌표
    level: 3 // 지도의 확대 레벨
    };  
...
linePath.push(new kakao.maps.LatLng(<%=markerList.get(i).getLATITUDE()%>, <%= markerList.get(i).getLONGITUDE()%>))
<%};%>
...
positions.push({title: '<div>제목 : <%=postList.get(i).getTitle()%></div> <div>내용 : <%=postList.get(i).getPost()%></div><div>작성일자 : <%=postList.get(i).getPost_dt()%></div>',
	latlng: new kakao.maps.LatLng(<%=markerList.get(i).getLATITUDE()%>, <%=markerList.get(i).getLONGITUDE()%>)})
<%};%>

<% for(int i =0; i<markerList.size(); i ++) {%>
shapes.push(<%=markerList.get(i).getMarker_shape()%>)
<%}%>
